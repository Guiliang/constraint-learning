...
from cirl_stable_baselines3.common.preprocessing import (get_action_dim,
                                                         is_image_space,
                                                         preprocess_obs)
from cirl_stable_baselines3.common.torch_layers import (BaseFeaturesExtractor,
                                                        FlattenExtractor,
                                                        MlpExtractor, NatureCNN,
                                                        create_mlp)
from cirl_stable_baselines3.common.utils import (get_device,
                                                 is_vectorized_observation)
from cirl_stable_baselines3.common.vec_env import VecTransposeImage
...
class ActorCriticPolicy(BasePolicy):
    ...
    :param latent_dim: the dimension of latent code
    ...
    def __init__(
        ...
        if latent_dim is not None:  # policy will be conditioned on latent codes (one-hot codes)
            new_low = np.concatenate([self.observation_space.low, np.zeros(latent_dim)], axis=0)
            new_high = np.concatenate([self.observation_space.high, np.ones(latent_dim)], axis=0)
