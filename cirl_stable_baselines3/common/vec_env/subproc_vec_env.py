    ...
    while True:
        try:
            ...
            if cmd == "step":
                ...
                observation, reward, done, info = env.step_with_code(data[0], data[1], data[2])
                if done:
                    # save final observation where user can get it, then reset
                    info["terminal_observation"] = observation
                    observation = env.reset()
            ...
            elif cmd == "reset_with_values":
                observation = env.reset(**data)
                remote.send(observation)
            elif cmd == "render":
                remote.send(env.render('human'))
            elif cmd == "close":
                env.close()
                remote.close()
                break
...
class SubprocVecEnv(VecEnv):
    ...
    def step_async_with_code(self, actions, codes):
        for remote, action, code in zip(self.remotes, actions, codes):
            remote.send(("step_with_code", (action, code, self.games_by_aids)))
        self.waiting = True
    ...
    def reset_with_values(self, info_dicts):
        for remote, info_dict in zip(self.remotes, info_dicts):
            remote.send(("reset_with_values", info_dict))
        obs = [remote.recv() for remote in self.remotes]
        return _flatten_obs(obs, self.observation_space)
